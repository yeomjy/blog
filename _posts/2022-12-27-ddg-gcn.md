...
![simplex](https://umap-learn.readthedocs.io/en/latest/_images/simplices.png)
*Image from: <https://umap-learn.readthedocs.io/en/latest/how_umap_works.html>*
...
and the **boundary map** $\partial$ is defined by
...
That is, to think $(X_{v_i})_j = f_j(v_i)$, for each $f_j: \mathcal{V} \rightarrow \R \simeq \R^{|\mathcal{V}|}$
