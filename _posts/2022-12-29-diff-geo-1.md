...
where $V$ is an open subset of $\R^n$. We say $n$ is the **dimension** of $M$, or simply, we say $M$ is $n$-manifold.
We call the pair $(U, \psi)$ a **coordinate chart**. and the collection of coordinate chart covers $M$, i.e.

$\{ U_{\alpha}, \psi_{\alpha} \}_{\alpha}$ such that
...
For $p \in M$, if there exists $\psi: U \rightarrow V \subset \mathbb{H}$ such that $\psi(p)_n = 0$, then we say $p$ is a boundary point of $M$ and denote $\partial M$ as a set of all boundary points of $M$.
...
Finally, for $f:M\rightarrow N$, the **differential map** $df_p: T_p M \rightarrow T_{f(p)}N$ is

$$
df_p (v)(\psi) = v (\psi \circ f)
$$

Of course, in terms of local coordinates of $M, N$, the matrix representation of $df_p$ is the jacobian matrix.
...
**$k$-alternating tensor** $T$ is a $k$-tensor such that 
...
**Differential $k$-form** on manifold $M$ is an assignment of $k$-alternating tensor for each $p \in M$.
